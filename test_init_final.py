...
def init():
	...
	for i in range(bossNum):
		bossTime.append(datetime.datetime.now()+datetime.timedelta(days=365, hours = 9, seconds = 10))
		tmp_bossTime.append(datetime.datetime.now()+datetime.timedelta(days=365, hours = 9, seconds = 10))
...
async def my_background_task():
	...
	while not client.is_closed:
		now = datetime.datetime.now() + datetime.timedelta(hours = 9, seconds = 10)
	...
	if client.get_channel(channel) != msg.channel :
		...
		##################################
			...
			if message.content.startswith(bossData[i][0] +'삭제'):
				bossTime[i] = datetime.datetime.now()+datetime.timedelta(days=365, hours = 9, seconds = 10)
				tmp_bossTime[i] =  datetime.datetime.now()+datetime.timedelta(days=365, hours = 9, seconds = 10)
				bossTimeString[i] = '99:99:99'
				bossDateString[i] = '9999-99-99'
		...
		if message.content.startswith('!ㅂ') or message.content.startswith('!q') or message.content.startswith('!ㅃ'):
			...
			for i in range(bossNum):
				if sorted_datelist[0] == bossTime[i]:
					leftTime = bossTime[i] - datetime.datetime.now() + datetime.timedelta(hours = 9, seconds = 10)
		...
		if message.content.startswith('!현재시간'):
			now3 = datetime.datetime.now() + datetime.timedelta(hours = 9, seconds = 10)
